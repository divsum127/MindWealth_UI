...
3. **Push** via **`divsum127` PAT** to **allowlisted remotes** after deploy + smoke pass. **Always** use `git-push-dev.sh` — never plain `git push origin`.
   ...
   ```bash
   SKILL_SCRIPTS=/home/ubuntu/uiv2/git/MindWealth_UI/.cursor/skills/robust-test-and-dev-deploy/scripts
...
```bash
cd /home/ubuntu/uiv2/git/MindWealth_UI
SKILL_SCRIPTS=.cursor/skills/robust-test-and-dev-deploy/scripts
