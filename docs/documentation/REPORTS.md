...
## 9. Data sources and how each report is produced

Reports are **generated by the MindWealth backend** (daily batch in `MindWealth/send_email.py` and related modules), then **copied** into this repo via `update_trade_data.sh`. MindWealth_UI does not re-run strategy math; it ingests, displays, consolidates (chatbot), and overlays (Conviction Engine).

### 9.1 Master pipeline (MindWealth backend)

```mermaid
flowchart TB
  subgraph inputs [Upstream inputs]
    YF[Yahoo OHLC via load_yf]
    CACHE[cache/US stock history]
    VAL[valuations.csv market cap]
  end
  subgraph strategies [Per-function strategy runs]
    FT[FRACTAL TRACK fib_ret]
    DD[DELTADRIFT distance]
    TP[TRENDPULSE trendline]
    BM[BAND MATRIX bollinger]
    Others[SIGMASHELL OSCILLATOR PULSEGAUGE BASELINEDIVERGE ...]
  end
  subgraph aggregate [Aggregated in send_email.py]
    ALL[all_long_dict_list + all_short_dict_list]
  end
  subgraph reports [Daily CSV outputs]
    AS[all_signal.csv]
    OS[outstanding_signal.csv]
    NS[new_signal.csv]
    TS[target_signal.csv]
    BR[breadth.csv]
    CP[combined_performance_report.csv]
    HN[horizontal_new_high_report.csv]
    FS[F-Stack-Analyzer.csv]
    CL[claude_signals_report txt+csv]
  end
  YF --> strategies
  CACHE --> strategies
  strategies --> ALL
...
**Common upstream object:** Each strategy module (e.g. `fib_ret.fib_ret_email`, `distance.distance_email`) returns **long/short signal dict lists** plus optional chart figures. `send_email.py` concatenates them into `all_long_dict_list` / `all_short_dict_list` (one dict per signal row, with `Function` set). Most aggregate reports are **filters or rollups** of that pool—not separate strategy runs.

| Stage | Where | What happens |
|-------|--------|----------------|
| Strategy scan | `MindWealth/*_email.py`, `helper_functions/*` | Backtest + live signals per asset/interval; OHLC from Yahoo/cache |
| Daily orchestration | `MindWealth/send_email.py` (main) | Runs all functions, builds shared dict lists, writes dated CSVs |
| Sync to UI repo | `update_trade_data.sh` | Copies `trade_store/US/*.csv` → MindWealth_UI |
| Chatbot consolidation | `chatbot/convert_signals_to_data_structure.py` | Splits outstanding/target/breadth; copies Claude TXT |
| UI / chatbot | Streamlit, `SmartDataFetcher` | Reads latest dated files; optional supplements (all_signal, virtual_trading) |

---

### 9.2 Per-report: sources and production

#### Outstanding Signals (`*_outstanding_signal.csv`)

| | |
|--|--|
| **Primary sources** | Merged `all_long_dict_list` + `all_short_dict_list` after all strategy runs |
| **Production** | `get_all_outstanding_signal()` → `send_outstanding_signal_mail()` |
| **Processing** | Filters by win rate ≥70%, trades ≥4, interval-based **signal-age retention** (5–70 days) or **exit within 3 days**; splits confirmed vs unconfirmed for email tables |
| **Output** | Single daily snapshot (not cumulative). See [§5.1 time window](#51-outstanding-signals-_outstanding_signalcsv) |
| **UI consolidation** | Open rows → `entry.csv`; closed → `exit.csv` via `convert_signal_file_to_data_structure.py` |

#### All Signal Report (`*_all_signal.csv`)

| | |
|--|--|
| **Primary sources** | Same merged dict lists as outstanding |
| **Production** | `get_all_signal()` — **no** outstanding retention filter; essentially all candidates from the run |
| **Processing** | Union of columns across functions; extra Trendpulse/divergence fields vs outstanding |
| **Significance** | Broadest per-run dump; used when chatbot/UI need signals missing from outstanding (see [byddy_deep_dive_missing_signals_fix.md](byddy_deep_dive_missing_signals_fix.md)) |

#### New Signals (`*_new_signal.csv`)

| | |
|--|--|
| **Primary sources** | Same merged dict lists |
| **Production** | `get_all_new_signal()` → `send_new_signal_mail()` |
| **Processing** | Rows where confirmation status is **not** `is` / `was CONFIRMED` (unconfirmed / pending) |
| **Significance** | “Brand new” setups awaiting confirmation—not the same as “newly added to outstanding” |

#### Portfolio / Target signal (`*_target_signal.csv`)

| | |
|--|--|
| **Primary sources** | Same merged dict lists |
| **Production** | `get_all_new_target_exit_signal()` → `send_new_target_exit_mail()` |
| **Processing** | Symbol must be in exit-target universe; parses **Targets** ladder (pivot, F-Stack, horizontal, EMA 200, backtest max/min); uses `load_yf` path from signal date → today; flags prices **near** targets (`PERCENTAGE_NEAR_EXTENSION` in MindWealth constants, typically ~90% of move to target); adds portfolio-specific columns |
| **Auxiliary data** | `./csv_output_data/valuations.csv` (market cap for sort) |
| **UI consolidation** | Appended to `portfolio_target_achieved.csv` (deduped) |

#### Signal Breadth Indicator (`*_breadth.csv`)

| | |
|--|--|
| **Primary sources** | `sent_body_df` — per-function signal DataFrames accumulated during the daily run |
| **Production** | `send_breadth_indicator_email()` → `compute.get_enhanced_breadth_signal()` + `trade_arrival_analysis` |
| **Processing** | Counts **new long/short signals** on S&P 500 universe; computes 6-month top-10% thresholds and **Today … Percentile From Top**; drops legacy Bullish % columns in current pipeline |
| **Significance** | Market-wide participation, not per-ticker positions |

#### Combined Performance Report (`*_combined_performance_report.csv`)

| | |
|--|--|
| **Primary sources** | Backtest / forward-test analytics (not the live signal dict lists) |
| **Production** | `send_combined_performance_report(today_date_str)` |
| **Processing** | Aggregates win %, holding period, profit distributions by **Strategy** × **Interval** × **Signal Type**; `Function` column often labels section (e.g. `Forward Testing`) |
| **Significance** | Strategy health dashboard, not trade blotter |

#### Horizontal & New High Report (`*_horizontal_new_high_report.csv`)

| | |
|--|--|
| **Primary sources** | **Horizontal:** `email_pages.horizontal.horizontal_email()` — proximity to horizontal levels. **New High:** `helper_functions.new_high._build_new_high_report()` |
| **Production** | `build_combined_horizontal_new_high_report_df()` merges both with `Report Type` column |
| **Processing** | Symbol-level price/level rows (not full signal schema) |
| **Also used by** | Claude prompts (per-symbol horizontal / new-high JSON attachments) |

#### F-Stack Analyzer (`F-Stack-Analyzer.csv`, US/India)

| | |
|--|--|
| **Primary sources** | `fib_ext.fib_ext_email()` (extension bands / Fib levels) |
| **Production** | Cached as `F_STACK_ANALYZER` in daily run; written before Claude step |
| **Processing** | Band ranges, next Fib extension vs latest price |
| **Also used by** | Claude per-box `fib_ext_report_dict` |

#### Virtual Trading (`virtual_trading_long.csv`, `virtual_trading_short.csv`)

| | |
|--|--|
| **Primary sources** | `virtual_trading.VirtualTrader` — rule-based simulated book |
| **Production** | Separate from main `send_email` signal mail path (maintained in trade_store) |
| **Processing** | Simpler position schema (entry/exit dates, P&L, status) |
| **UI** | Virtual Trading page; chatbot may supplement entry loads; Conviction Engine `SIGNAL_SOURCES` |

#### Trade Details (nested CSVs)

| | |
|--|--|
| **Primary sources** | Per-function backtest artifacts under `success_rate/`, `forward_testing/`, `latest_performance/` |
| **Production** | MindWealth backend (not rebuilt in UI repo) |
| **Processing** | One CSV per `{function}/{asset}/{interval}.csv` — time series / trade history for charts |

#### Per-function exports (`*_Trendline.csv`, `*_Fib-Ret.csv`, …)

| | |
|--|--|
| **Primary sources** | Same strategy run as main pipeline |
| **Production** | Each `*_email()` also writes dated `trade_store/US/YYYY-MM-DD_{FunctionFile}.csv` |
| **UI** | Chart deep-dives only (`src/components/charts.py` mapping) |

---

### 9.3 Claude Shortlisted Signal — sources and how the report is built

Claude produces **two artifacts** each US daily run:

| Artifact | Path | Role |
|----------|------|------|
| **TXT** (narrative) | `*_claude_signals_report.txt` → `chatbot/data/claude_report.txt` | Executive synthesis, themes, warnings; pipe-delimited shortlist `Symbol,Interval,Function,Date,Direction` |
| **CSV** (tabular) | `*_claude_signals_report.csv` | Outstanding-style rows for signals Claude selected—Streamlit cards |

**MindWealth_UI does not call Claude to regenerate the report**; it only **displays** and **feeds TXT to the chatbot** (`claude_report` signal type).

#### Step-by-step production (MindWealth `send_email.py`)

```mermaid
flowchart TB
  subgraph inputs [Claude inputs]
    ENT[entry_signal_symbol_dict from outstanding mail]
    FIL[filter_signals removes late entries]
    MKT[market_cap_pe from get_all_market_cap_pe_ratio]
    PF[portfolio_report_dict VirtualTrader]
    FIB[fib_ext_report_dict F-Stack]
    HOR[horizontal_report_dict]
    NH[new_high_report_dict]
    POOL[all_long_dict_list + all_short_dict_list full pool]
  end
  subgraph parallel [Parallel Claude calls NUM_CLAUDE=8]
    B1[claude_1 box prompt]
    B2[claude_2 ...]
    BN[claude_8 ...]
  end
  SYN[Synthesis prompt merges groups]
  OUT[TXT + pipe-delimited list]
  CSV[CSV rows matched from pool]
  ENT --> FIL
  FIL --> BOX[claude_box_classification by symbol]
  BOX --> B1
  BOX --> B2
  BOX --> BN
  MKT --> B1
  ...
  BN --> SYN
  SYN --> OUT
  OUT --> PARSE[Parse Symbol Interval Function Date Direction]
  PARSE --> CSV
  POOL --> CSV
```
...
|------|-------------------|--------------|
| 1 | `send_outstanding_signal_mail` | Builds **outstanding** tables; populates `entry_signal_symbol_dict` (symbol → list of row values + `HEADER`) |
| 2 | `filter_signals()` | Drops entries that moved too far from signal price (3% Daily / 5% Weekly / 10% Monthly+) before Claude sees them |
| 3 | `claude_box_classification()` | Splits symbols into **8** groups (`NUM_CLAUDE` in `MindWealth/constant.py`) for parallel API calls |
| 4 | `claude_box_prompt()` | For each group, builds JSON prompt: **`GOOD_SIGNAL_QUERY`** rules + **entry signals** for symbols in that box + optional **Portfolio**, **Horizontal**, **New High**, **F-Stack** excerpts (filtered to box symbols) + **market cap / PE** string |
| 5 | `claude_main_func()` (`claudeai_agent.py`) | Runs **8 parallel** Claude (or configured) completions, then **synthesis** prompt to merge into one narrative |
| 6 | `send_claude_analysis_email()` | Parses `|||` … `|||` section in model output into tuples `symbol,interval,function,date,direction`; **joins back** to `all_long_dict_list` + `all_short_dict_list` for matching rows → **body_df1/body_df2** (CSV); emails HTML; returns **output_text** (TXT) |
| 7 | File write | `save_text_to_file` → `YYYY-MM-DD_claude_signals_report.txt`; `concat(body_df1, body_df2)` → `YYYY-MM-DD_claude_signals_report.csv` |

**Prompt content (`GOOD_SIGNAL_QUERY` in `MindWealth/constant.py`):** Multi-function quant synthesis—cycle context, lateness vs targets, Sharpe/CAGR/win-rate gates, conflict resolution across TrendPulse / FRACTAL TRACK / etc. Conservative, trader-ready shortlist.

**What Claude does *not* receive:** Raw `all_signal.csv` in full (unless those rows are also in outstanding + pass `filter_signals`). Breadth SBI, combined performance tables, and exit history are not primary Claude inputs unless reflected in attached reports.

#### MindWealth_UI consumption

| Consumer | Data used | Processing |
|----------|-----------|------------|
| **Claude Shortlisted Signal** page | Latest `*_claude_signals_report.{txt,csv}` | TXT rendered as markdown; CSV via `parse_outstanding_signal` |
| **AI Chatbot** (`claude_report` type) | `chatbot/data/claude_report.txt` (copy only) | `DataProcessor.load_claude_report()` — full text appended to LLM context; **no column extraction** |
| **Converter** | Latest `*_claude_signals_report.txt` | `shutil.copy2` → `claude_report.txt` (no transformation) |

---

### 9.4 In-app reports (not in `trade_store`)

| Report | Data sources | Production |
|--------|--------------|------------|
| **Conviction Engine Daily Report** | `conviction_store/{TICKER}.json` (yfinance fundamentals) + latest signal CSVs for universe | `generate_daily_report()` in `src/conviction_engine/engine.py` at runtime |
| **Conviction overlays** | Same signal files + JSON store | `scripts/update_conviction_fundamentals.py` → optional `conviction_store/overlays/*_conviction.csv` |
| **Monitored Trades** | `monitored_trades.json` (user-curated) | UI only |

---

### 9.5 Quick reference table

| Report | Upstream data | Main transform | Output in repo |
|--------|---------------|----------------|----------------|
| outstanding_signal | Strategy dict lists | Retention + quality filters | `trade_store/US/YYYY-MM-DD_outstanding_signal.csv` |
| all_signal | Strategy dict lists | None (full run) | `*_all_signal.csv` |
| new_signal | Strategy dict lists | Unconfirmed only | `*_new_signal.csv` |
| target_signal | Strategy dict lists + OHLC path + targets | Near-target math | `*_target_signal.csv` |
| breadth | `sent_body_df` per function | SBI trade-arrival counts | `*_breadth.csv` |
| combined_performance | Backtest DB / analytics | Aggregate stats | `*_combined_performance_report.csv` |
| horizontal_new_high | Horizontal + new-high modules | Combine + tag type | `*_horizontal_new_high_report.csv` |
| F-Stack | fib_ext | Band/Fib table | `*_F-Stack-Analyzer.csv` |
| **Claude TXT** | Filtered entries + side reports + **GOOD_SIGNAL_QUERY** | 8-way parallel LLM + synthesis | `*_claude_signals_report.txt` |
| **Claude CSV** | Same pool as strategies | Subset matching Claude `|||` list | `*_claude_signals_report.csv` |
| entry/exit (chatbot) | outstanding (+ all_signal in converter) | Split open/closed, dedupe, append | `chatbot/data/entry.csv`, `exit.csv` |
...
| [byddy_deep_dive_missing_signals_fix.md](byddy_deep_dive_missing_signals_fix.md) | Why outstanding vs all_signal vs entry.csv can disagree per ticker |
