...
### SSI VIX ratio formula/threshold inversion (found 2026-08-02 investigation)

**VIX-RATIO-01: Fix SSI `vix_ratio` orientation so normal contango is not painted red as “stress/backwardation”**
- **What**: `yahoo_inputs.vix_ratio_series()` returns `VIX3M/VIX` (~1.05–1.15 in calm markets) while `layer2.py` / `superindex.py` / `SSI_CONFIG.yaml` treat `≥1.05` as stress and `≤0.95` as complacency — the VIX/**VIX3M** (spot÷3M) interpretation. Docs (`SSI_OpenQuestions_Understanding.md`: “VIX > VIX3M = backwardation”) and friday checklist name (`VIX/VIX3M`) agree with thresholds, **not** with the formula. Live evidence: `positioning.json` `vix_ratio=1.09` → `signal: "stress"` → Vue `var(--red)`.
- **Fix options** (pick one, then rebuild SSI history): (A) change formula to `df["vix"] / df["vix3m"]`; or (B) keep VIX3M/VIX and invert thresholds (`stress_max: 0.95`, `complacency_min: 1.05`) + rename signals. Prefer (A) to match docs/checklist and Rohit’s mental model. Also fix cron comment (`08:00 ET` vs actual `08:00 UTC`) and ensure VIX level display is 2dp.
- **Files**: `src/sentiment_superindex/data/yahoo_inputs.py`, `src/sentiment_superindex/engine/layer2.py`, `src/sentiment_superindex/engine/superindex.py`, `macro_intelligence/SSI_CONFIG.yaml`, tests under `tests/test_ssi_*.py`, rebuild via `scripts/run_ssi_daily.py` / history rebuild
- **Status**: OPEN — investigated 2026-08-02; not coded yet
...
2. **Investigate: Rohit VIX ratio 1.06 red-backwardation vs Yahoo ~1.01 (June 24–25 report)** — SUCCESSFUL (investigation only; bug still open)
   - Summary: Root-caused the June 25 WhatsApp report (site ratio 1.06 flashed red as backwardation; phone check ~1.01; site VIX higher than Yahoo). **Primary bug:** SSI `vix_ratio` is computed as `^VIX3M ÷ ^VIX` (same as Runic VXTS / normal contango ~1.05–1.15) but Layer-2 thresholds + docs treat `≥1.05` as **stress/backwardation** — inverted vs the intended `VIX/VIX3M` convention (`SSI_OpenQuestions_Understanding.md`, friday checklist label `VIX/VIX3M`). Live `positioning.json` (2026-07-31) still shows `vix_ratio=1.094` with `signal: "stress"`. Vue paints `signal==='stress'` as `var(--red)`. Secondary: SSI cron is `0 8 * * 1-5` on **UTC** host (not 08:00 ET as config comment claims) → prior-session Yahoo closes; portfolio showed unrounded VIX `18.709999…`. Visitor “1.01” may also be VIX3M−VIX spread (Rohit’s own hypothesis). **Not fixed** — no prior job-status root-cause; still open gap. Fix path: invert formula to `VIX/VIX3M` *or* swap stress/complacency thresholds to match VIX3M/VIX; align cron TZ comment; round VIX display.
   - Files inspected (no code changes): `src/sentiment_superindex/data/yahoo_inputs.py`, `src/sentiment_superindex/engine/layer2.py`, `src/sentiment_superindex/engine/superindex.py`, `macro_intelligence/SSI_CONFIG.yaml`, `src/macro_intelligence/data/yahoo_pull.py`, `MindwealthUI_Vue/server/utils/sentiment-mapper.ts`, `instruction_docs/chat_ques/All of Us - last month (21 Jun - 21 Jul 2026) - STATUS.md` (was ❌ PENDING)
   - Prod impact: none this session (investigation only). Fix later = merge + SSI daily rebuild.

3. **Move % Above 200DMA from SSI Layer 1 (Weekly Pulse) to Layer 2 (Daily Timing)** — SUCCESSFUL
   - Summary: Per architecture doc, `% above 200-day MA` is a Layer 2 daily breadth signal. Moved `pct_above_200dma` from `ssi_score.layers.layer1` into `layer2` in config, superindex builder, and positioning display inputs. Alpha Terminal Sentiment UI reads `GET /api/v1/analytics/sentiment/layers` → `inputs.layer2`, so the metric now renders under Daily Timing.
   - Files changed: `macro_intelligence/SSI_CONFIG.yaml`, `src/sentiment_superindex/engine/superindex.py`, `src/sentiment_superindex/engine/positioning.py`, `macro_intelligence/DATA_SOURCES.yaml`, `docs/mindwealth-api-docs/services/analytics/endpoints/get-sentiment-layers.md`, `tests/test_ssi_superindex.py`, `tests/test_ssi_display_rounding.py`
