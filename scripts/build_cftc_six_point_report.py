...
def build_markdown(data: dict) -> str:
    ...
    lines += [
        ...
        "",
        "**On the GFC specifically:** this has now been stated three different ways, so to close it:",
        "the 2008 episodes are **not** in the percentile grids. They appeared there only because the",
        "percentile helper would rank a 20-observation window and label it a 3-year percentile. It now",
        f"requires a full window, so the first rankable week is {diag['first_full_window']} — after the",
        "crash. Including 2008 is a methodology decision (shorter window, accepted partial window, or the",
        "legacy proxy), not a data-availability problem.",
        "",
